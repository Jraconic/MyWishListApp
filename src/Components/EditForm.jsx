...
export default function EditForm({ item, onEditItem, onCancel }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Pass the item ID and the updated form data
            await onEditItem(item.id, formData);
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-80 z-50 flex justify-center items-center p-4">
            <div className="bg-stone-50 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-lg overflow-y-auto max-h-full">
                <h2 className="text-2xl font-light mb-6 text-gray-900 tracking-wider uppercase border-b pb-2 border-amber-500">Edit Item: {item.name}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        <button
                            type="submit"
                            className="px-6 py-2 bg-amber-600 text-white font-medium rounded-lg shadow-md hover:bg-amber-700 transition duration-150 disabled:opacity-50"
                            disabled={isSubmitting}
                        >
